...
export function normalizeBooking(booking) {
  ...
  return {
    ...
    centreName,
    farmerName: booking.farmer_name || booking.farmerName || '',
    farmer_name: booking.farmer_name || booking.farmerName || '',
    farmerId: booking.farmer_id || booking.farmerId || booking.user_id || booking.userId || null,
    user_id: booking.user_id || booking.userId || booking.farmer_id || booking.farmerId || null,
    mobile: booking.mobile || booking.phone || '',
    aadhaarLast4: booking.aadhaar_last4 || booking.aadhaarLast4 || '',
...
/**
 * Creates a real booking via POST /api/bookings with direct Supabase resilience fallback.
 * Token is generated and persisted in the database.
 */
export async function createBooking({
  ...
  try {
    ...
    console.warn('[apiService] Express API booking route threw error, engaging direct cloud registry fallback:', apiErr.message);
    
    // Direct cloud client fallback
    const { createSlotBooking } = await import('./supabaseService');
    const directRes = await createSlotBooking(payload);
